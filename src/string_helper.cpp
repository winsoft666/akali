...
#if (defined _WIN32 || defined WIN32)
static wchar_t *InnerMakeStringW(int &iBufLen, int iBufSizeStep, const wchar_t *pstrFormat,
                                 va_list Args) {
  iBufLen = 0;
  if (!pstrFormat)
    return NULL;
...
static char *InnerMakeStringA(int &iBufLen, int iBufSizeStep, const char *pstrFormat,
                              va_list Args) {
  iBufLen = 0;
  if (!pstrFormat)
    return NULL;
